...
export async function updateJournalEntry(input: {
  ...
  if (result.rows.length === 0) {
    throw new Error(
      "Journal entry not found or you don't have permission to edit it.",
    );
  }
...
export async function getBookJournal(
  bookId: number,
): Promise<BookJournal | null> {
  const user = await getCurrentUser();
...
// Get books that can be attached to a journal entry
export async function getJournalBookOptions(): Promise<JournalBookOption[]> {
  const user = await getCurrentUser();

  if (!user || !user.userId || !user.profileId) {
    throw new Error("You must be signed in to choose a book for your journal.");
  }
  ...
  const result = await queryWithContext(
    ...
    ORDER BY b.id, sb.updated_at DESC`,
    [user.profileId],
  );

  return result.rows as JournalBookOption[];
}
