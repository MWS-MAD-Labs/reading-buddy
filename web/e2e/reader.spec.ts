...
test.describe("E-Reader Functionality", () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const readLink = page
      .locator('a[href*="/dashboard/student/read/"]')
      .first();
    ...
    await Promise.all([
      page.waitForURL(new RegExp(`/dashboard/student/read/${bookId}`), {
        timeout: 30000,
      }),
      readLink.click(),
    ]);
  ...
  test("should load the reader and display book content", async ({ page }) => {
    ...
    await page.waitForSelector("text=Loading book...", {
      state: "hidden",
      timeout: 30000,
    });
    ...
    await page.waitForSelector(".reader-page-content", {
      state: "attached",
      timeout: 30000,
    });
    ...
    await expect(
      page.locator('span:has-text("📍 Page")').first(),
    ).toBeVisible();
  });
  ...
  test("should navigate between pages", async ({ page }) => {
    ...
    await expect(
      page.locator('span:has-text("📍 Page")').first(),
    ).toContainText("Page 3", { timeout: 10000 });
    ...
    await expect(
      page.locator('span:has-text("📍 Page")').first(),
    ).toContainText("Page 1", { timeout: 10000 });
  });
  ...
  test("should persist reading progress after reload", async ({ page }) => {
    ...
    await expect(
      page.locator('span:has-text("📍 Page")').first(),
    ).toContainText("Page 3", { timeout: 15000 });
  });
  ...
  test("should open and adjust reading settings", async ({ page }) => {
    ...
    // All PDF books now use the rendered-page reader with zoom controls
